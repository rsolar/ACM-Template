...
int Relation(Point p, Line
             L) { // 计算点p与直线L的相对关系 ,返回ONLINE,LEFT,RIGHT
...
int InsideConvex(Point p,
                 const Polygon &poly) { // 判断点p是否在凸多边形poly内
  Point q(0, 0);
  Line side;
  int i, n = poly.size();
  for (i = 0; i < n; i++) {
...
int InsidePolygon(const Polygon &poly,
                  Point p) { // 判断点p是否在简单多边形poly内, 多边形可以是凸的或凹的
  int i, n, count;
  Line ray, side;
  n = poly.size();
  count = 0;
...
bool InsidePolygon(const Polygon &poly,
                   Line L) { // 判断线段是否在多边形内 (线段的点可能在多边形上)
  bool result;
  int n, i;
  Points points;
  Point p;
...
bool IsInCircle(const Circle &C,
                const Rect_2 &rect) { //判断圆是否在矩形内(不允许相切)
