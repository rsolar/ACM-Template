...
struct Point {
  ...
  //极角排序用
  bool sign()const { return y > 0 || (y == 0 && x > 0); }
  bool operator<(const Point &b)const {
    return sign() != b.sign() ? sign() > b.sign() : (*this ^ b) > 0;
  }
  //绕原点逆时针旋转角度r(弧度)
  void transXY(double r) {
    double tx = x, ty = y;
    x = tx * cos(r) - ty * sin(r);
    y = tx * sin(r) + ty * cos(r);
  }
...
struct Line {
  ...
  //求两直线交点
  //第一个值为0表示直线重合, 为1表示平行, 为2表示相交, 只有第一个值为2时, 交点才有意义
...
//判断线段相交
bool segInter(const Line &l1, const Line &l2) {
...
//判断直线l1和线段l2相交
bool lineSegInter(const Line &l1, const Line &l2) {
  return sgn((l2.s - l1.e) ^ (l1.s - l1.e)) * sgn((l2.e-l1.e) ^ (l1.s - l1.e)) <= 0;
}
//点到直线距离
//返回为直线上到该点最近的点
Point nearestPointToLine(const Point &P, const Line &L) {
...
//判断点在线段上
bool pointOnSeg(const Point &P, const Line &L) {
...
//计算多边形面积
//点的编号为0 ~ n - 1
double calcPolyArea(Point p[], int n) {
  double res = 0;
  for (int i = 0; i < n; i++) { res += (p[i] ^ p[(i + 1) % n]) * 0.5; }
  return fabs(res);
}
//判断点在凸多边形内
...
//0: 点在凸多边形边上
//1: 点在凸多边形内
int pointInConvexPoly(const Point &a, Point p[], int n) {
  for (int i = 0; i < n; i++) {
    if (sgn((p[i] - a) ^ (p[(i + 1) % n] - a)) < 0) { return -1; }
    else if (pointOnSeg(a, Line(p[i], p[(i + 1) % n]))) { return 0; }
  }
  return 1;
}
//判断点在任意多边形内
//射线法, poly[]的顶点数要>=3, 点的编号为0 ~ n - 1
//返回值
//-1: 点在凸多边形外
//0: 点在凸多边形边界上
//1: 点在凸多边形内
int inPoly(const Point &p, Point poly[], int n) {
  int cnt = 0; Line ray(p, Point(-INF, p.y));
  for (int i = 0; i < n; i++) {
...
//判断凸多边形
//允许共线边, 点可以是顺时针给出也可以是逆时针给出, 点的编号为0 ~ n - 1
